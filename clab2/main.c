...
// You must complete this function.
    ...
    for (int i = 0; i < csize; i++) {
        if (c[i]->code == p->code) {
            printf("Class code already have. stop addition.\n");
            free(p); // Free allocated memory before returning
            return csize;
        }
    }
...
int applyMyClasses(int my[], int msize, struct st_class* c[], int csize) {
    ...
    while (1) {
        printf(">> Enter a class code (0 to finish): ");
        scanf("%d", &code);

        if (code == 0) {
            break; // Finish input loop
        }
        ...
        if (Duplicate) {
            printf("Class with code %d is already in your classes.\n", code);
        } else {
            // Add the code to my classes
            my[msize] = code;
            msize++;
            printf("Class with code %d added to your classes.\n", code);
        }
    }
...
void printMyClasses(int my[], int msize, struct st_class* c[], int csize) {
    ...
    for (int i = 0; i < msize; i++) {
        ...
        if (p != NULL) {
            printf("[%d] %s [credits %d - %s]\n", p->code, p->name, p->unit, (p->grading == 1) ? "Grade" : "P/F");
        }
    }
}
...
void saveMyClass(int my[], int msize, struct st_class* c[], int csize) {
    ...
    // Write header
    fprintf(fp, "Class Code\tClass Name\tCredits\tGrading\n");

    // Write each class information
    for (int i = 0; i < msize; i++) {
        int code = my[i];
        // Find the class with the given code
        for (int j = 0; j < csize; j++) {
            if (c[j]->code == code) {
                ...
                fprintf(fp, "%d\t%s\t%d\t%s\n", c[j]->code, c[j]->name, c[j]->unit, (c[j]->grading == 1) ? "Grade" : "P/F");
                break;
            }
        }
    }
    ...
    fclose(fp);
    printf("My classes saved successfully.\n");
}
